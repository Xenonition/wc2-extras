...
function enemy.do_gold(cfg, side)
	-- WC2X: Income-based economy replaces lump-sum gold dump.
	-- Enemy gets moderate starting gold + per-turn income from villages.
	local enemy_power = (wml.variables["wc2_difficulty.enemy_power"] or 6)
	local wc2x_cfg = wc2x and wc2x.config
	if wc2x_cfg then
		local difficulty_scale = wc2x_cfg.difficulty_gold_scale[enemy_power] or 1.0
		...
		local base_income = math.max(
			...
			math.ceil(10 * nplayers * difficulty_scale)
		)
		side.base_income = base_income
	else
		-- Fallback to original WC2 formula
