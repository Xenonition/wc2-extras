...
-- Manages permanent upgrades purchased in the shop that carry across scenarios.
-- State is stored in WML variables (wc2x_upgrades.*) which persist via savegame.
...
-- Read the current count of a purchased upgrade for a side
function upgrades.get_count(side_num, upgrade_id)
	local side = wesnoth.sides[side_num]
	return side.variables["wc2x_upgrades." .. upgrade_id] or 0
end

-- Increment the purchase count of an upgrade for a side
...
-- Get the price for the next purchase of an upgrade
function upgrades.get_price(side_num, upgrade_id)
	local count = upgrades.get_count(side_num, upgrade_id)
	local base = config.upgrade_prices[upgrade_id]
	if not base then return 999999 end
	return math.ceil(base * (config.upgrade_price_escalation ^ count))
end

-- Apply all purchased upgrades at scenario start for a human side
function upgrades.apply_for_side(side_num)
	...
	-- Unit discount — stored as wc2x_unit_discounts.TYPE = amount
	-- Applied via recruit event (see below)

	-- Reinforcements — spawn free units outside castle on turn 1
...
-- Apply castle hex upgrades (must happen during map generation or prestart)
function upgrades.apply_castle_hexes(side_num)
	...
	-- Find empty hexes adjacent to existing castle
	...
	-- Find hexes adjacent to castle that aren't already castle
	...
	-- Place supply villages (castle hex + village overlay)
	...
	-- Place barracks (castle hex, marked via side variable for spawn logic)
	...
	-- Place training grounds (castle hex, marked for XP logic)
...
-- Apply all upgrades at prestart for all human sides
	...
	for side_num, count in pairs(upgrades.pending_reinforcements) do
		...
		if leader then
			...
			for _, hex in ipairs(adj_hexes) do
				...
				local existing = wesnoth.units.get(hex.x, hex.y)
				if not existing then
					local utype = recruit_list[mathx.random(#recruit_list)]
...
-- Training ground XP: units ending turn on a training ground hex gain XP
...
-- Barracks: spawn a unit every N turns
on_event("side turn", function(cx)
	...
	for i = 1, barracks_count do
		...
		if bx and by then
			local existing = wesnoth.units.get(bx, by)
			if not existing then
				local utype = recruit_list[mathx.random(#recruit_list)]
				wesnoth.wml_actions.unit {
					side = side_num,
...
-- Unit discount: reduce recruit cost for specific unit types
-- Stored as wc2x_unit_discount.<unit_type_id> = discount_amount
on_event("recruit", function(cx)
	...
	local side = wesnoth.sides[side_num]
	local discount = side.variables["wc2x_unit_discount." .. u.type] or 0
	if discount > 0 then
		side.gold = side.gold + discount
	end
end)
